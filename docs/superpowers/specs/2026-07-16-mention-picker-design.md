...
- Dès la première lettre après `@`, la recherche devient **récursive sur tout le vault**
  (plus la liste plate des racines externes), fuzzy sur le **chemin complet**.
- **Une fois descendu dans un dossier, la recherche est contrainte à ce sous-arbre** :
  après `@Cours/`, taper `ja` cherche récursivement sous `Cours/` seulement, pas dans tout
  le vault. Le token porte donc toujours son propre périmètre, ce qui rend la descente
  utile (sinon elle ne servirait qu'à taper plus de lettres). Effacer jusqu'au `/`
  précédent (Backspace) remonte d'un niveau et rend la recherche à nouveau globale.
...
- Résultats plafonnés à 30 (comme `openNotePicker` aujourd'hui).
...
- **Dossier sélectionné → on descend dedans** : le token devient `@Cours/` et la liste
  affiche son contenu. Un dossier ne s'attache jamais.
...
- **Recherche récursive** : parcours en arrière-plan lancé à la première ouverture du
  picker, mis en cache pour la session, invalidé sur le `mtime` de la racine. Plafonné à
  **profondeur 4** et **2000 entrées**, dossiers cachés et `node_modules` ignorés. Les
  résultats du vault s'affichent immédiatement, ceux du disque se greffent à l'arrivée.
